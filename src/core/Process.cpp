...
void Process::waitForTickCompletion() {
  ...
  // Wait with timeout to prevent infinite freeze
  if (!m_cvKernel.wait_for(lock, std::chrono::milliseconds(100), [this] { return m_tickCompleted; })) {
    std::cerr << "Warning: Process " << m_pid << " timed out waiting for tick completion!" << std::endl;
  }
...
void Process::run() {
  // std::cout << "[DEBUG] Thread for P" << m_pid << " started." << std::endl;
  while (!m_stopThread) {
